...
//        $password = md5($password);
    ...
    function contact_us() {
        ...
        if(isset($_POST['contact-submit'])) {
            ...
            $msg = "Hello,<br/><br/>$comments<br/><br/>Regards,<br/>$fullname<br/>";
...
//                $_SESSION['download_attachment'] = "true";
                ...
                if($note_sell_mode == 0 or $the_user_id === $seller_id) {
                    ...
                    download_attachment($attachment_path,$attachment_path_name);
                }
                else {
                    $check_already_downloaded_sql = "SELECT * FROM downloads WHERE downloaded_note_id = '{$note_id}' AND downloader = '{$the_user_id}'";
    ...
    function download_attachment($download_path,$note_name="") {
//        $path = $download_path;
        $path_array = explode(",", $download_path);
        $path_name_array = explode(",", $note_name);
        ...
        if(sizeof($path_array) == 1) {
            ...
            $zipname = "Attachment_".time().".zip";
            $zip = new ZipArchive;
            $zip->open($zipname, ZipArchive::CREATE);
            $i=0;
            foreach ($path_array as $file) {
              $zip->addFile($file,$path_name_array[$i]);
                $i += 1;
            }
            $zip->close();
...
//            if(is_uploaded_file($_FILES['note_pdf']['tmp_name']))
            if(!empty(array_filter($_FILES['note_pdf']['name']))){
                
                foreach($_FILES['note_pdf']['name'] as $id=>$val){
                    
                    $note_pdf = $_FILES['note_pdf']['name'][$id];
                    $note_pdf_tmp = $_FILES['note_pdf']['tmp_name'][$id];
            ...
            if($error_check === 1){
                ...
                $note_attach_sql = "INSERT INTO seller_notes_attachment(attachment_note_id, attached_file_name, attached_file_path, created_by, modified_by) VALUES ('{$the_note_id}','{$note_pdf_name}','{$attachement_path_to_store}','{$the_user_id}','{$the_user_id}')";
...
//            $note_pdf_tmp = $_FILES['note_pdf']['tmp_name'];
            ...
            if($error_check === 1){
                ...
                if(!empty(array_filter($_FILES['note_pdf']['name']))){
                    ...
                    $unlink_path = explode(",", $path);
                    
                    foreach($unlink_path as $path) {
                      unlink($path);
                    }
                    ...
                    $update_attachment_sql = "UPDATE seller_notes_attachment SET attached_file_name='{$note_pdf_name}',attached_file_path='{$attachement_path_to_store}',modified_date=now(),modified_by=$the_user_id WHERE attachment_id=$attachment_id";
    ...
    function update_admin_profile() {
        if(isset($_POST['edit_admin_profile_submit'])){
            ...
            if($error_check === 1) {
                ...
                echo "<script>alert('Your Profile Updated successfully.');window.location.href = 'Dashboard-admin.php';</script>";
            }
