...
//        $note_published_date = $row['note_published_date'];
        ...
        if(empty($note_image)) {
            $select_default_note_sql = "SELECT * FROM system_config";
            $select_default_note_result = query($select_default_note_sql);
            confirmQuery($select_default_note_result);
            $results_default_note = array();
            while($default_note_img_row = fetch_array($select_default_note_result)) {
                $results_default_note[$default_note_img_row['key']] = $default_note_img_row['value'];
            }
            $note_image = $results_default_note['DefaultNoteDisplayPicture'];
        }
                        ...
                        <?php
                            ...
                            if(row_count($select_reviews_result) > 0) {
                                ...
                                while($reviewers_row = fetch_array($select_reviews_result)) {
                                    ...
                                    $reviewer_profile_picture = $reviewers_row['user_profile_picture'];
                                    
                                    if(empty($reviewer_profile_picture)) {
                                        $reviewer_default_dp_sql = "SELECT * FROM system_config";
                                        $reviewer_default_dp_result = query($reviewer_default_dp_sql);
                                        confirmQuery($reviewer_default_dp_result);
                                        $results_dp = array();
                                        while($default_dp_row = fetch_array($reviewer_default_dp_result)) {
                                            $results_dp[$default_dp_row['key']] = $default_dp_row['value'];
                                        }
                                        $reviewer_profile_picture = $results_dp['DefaultMemberDisplayPicture'];
                                    } 
